...
const defaultUsers = [
    {
        id: 1,
        fullName: "ကိုစိုင်းခမ်း",
        phone: "0977767776",
        email: "saikham@gmail.com",
        password: "password123",
        memberTier: "VIP Gold Member"
    }
];

let users = JSON.parse(localStorage.getItem("ai_mobile_users"));
if (!users || !Array.isArray(users) || users.length === 0) {
    users = defaultUsers;
    localStorage.setItem("ai_mobile_users", JSON.stringify(users));
}

let currentUser = JSON.parse(localStorage.getItem("ai_mobile_current_user")) || null;
...
function updateAuthUI() {
    ...
    const contactTitle = document.getElementById("contact-account-title");
    const contactSub = document.getElementById("contact-account-sub");
    const contactBtnText = document.getElementById("contact-account-btn-text");

    if (currentUser) {
        if (headerName) headerName.innerText = currentUser.fullName.split(" ")[0];
        if (contactTitle) contactTitle.innerText = currentUser.fullName + " (" + currentUser.memberTier + ")";
        if (contactSub) contactSub.innerText = "Phone: " + currentUser.phone + " • VIP Privileges Active";
        if (contactBtnText) contactBtnText.innerText = "အချက်အလက် ကြည့်ရန် (Manage Account)";
    } else {
        if (headerName) headerName.innerText = "Login";
        if (contactTitle) contactTitle.innerText = "AI Mobile VIP Account";
        if (contactSub) contactSub.innerText = "Login or Sign up for 10% discount on accessories";
        if (contactBtnText) contactBtnText.innerText = "Login / Sign Up (အကောင့်ဝင်/ဖွင့်ရန်)";
    }
}
...
function setAuthTab(tab) {
    ...
    if (tab === 'login') {
        tabLogin.className = "flex-1 py-2 text-xs font-bold rounded-lg bg-white shadow-sm text-indigo-600 transition-all";
        tabSignUp.className = "flex-1 py-2 text-xs font-bold rounded-lg text-slate-500 transition-all";
        formLogin.classList.remove("hidden");
        formSignUp.classList.add("hidden");
    } else {
        tabSignUp.className = "flex-1 py-2 text-xs font-bold rounded-lg bg-white shadow-sm text-indigo-600 transition-all";
        tabLogin.className = "flex-1 py-2 text-xs font-bold rounded-lg text-slate-500 transition-all";
...
function handleLoginSubmit(e) {
    ...
    const user = users.find(u => (u.phone === identifier || (u.email && u.email.toLowerCase() === identifier.toLowerCase())) && u.password === password);
    if (!user) {
        showAuthError("ဖုန်းနံပါတ် သို့မဟုတ် စကားဝှက် မှားယွင်းနေပါသည်");
        return;
    }

    currentUser = user;
    localStorage.setItem("ai_mobile_current_user", JSON.stringify(currentUser));
    updateAuthUI();
    closeAuthModal();
}

function handleDemoLogin() {
    const demoUser = users.find(u => u.phone === "0977767776") || users[0];
    if (demoUser) {
...
function handleSignUpSubmit(e) {
    ...
    if (password.length < 4) {
        showAuthError("စကားဝှက် အနည်းဆုံး ၄ လုံး ထားပေးပါ");
        return;
    }

    if (password !== confirmPassword) {
        showAuthError("စကားဝှက် နှစ်ခု မတူညီပါ၊ ပြန်လည်စစ်ဆေးပါ");
        return;
    }

    const existing = users.find(u => u.phone === phone);
    if (existing) {
        showAuthError("ဤဖုန်းနံပါတ်ဖြင့် အကောင့်ဖွင့်ပြီးသား ဖြစ်ပါသည်");
        return;
    }
...
function openProfileModal() {
    ...
    document.getElementById("profile-avatar").innerText = currentUser.fullName.charAt(0).toUpperCase();
    document.getElementById("profile-name").innerText = currentUser.fullName;
    document.getElementById("profile-tier").innerText = currentUser.memberTier;
    document.getElementById("profile-phone").innerText = currentUser.phone;
    document.getElementById("profile-email").innerText = currentUser.email || "မထည့်သွင်းထားပါ";
    document.getElementById("profile-fav-count").innerText = favourites.length + " items";
...
function handleLogout() {
    currentUser = null;
    localStorage.removeItem("ai_mobile_current_user");
    updateAuthUI();
    closeProfileModal();
}
